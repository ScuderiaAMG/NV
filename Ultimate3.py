...
try:
    ...
    from sg2im.data.utils import load_vocab
except ImportError:
    print("Warning: GAN/Diffusion libraries not available. Please install required packages.")
...
#     """
#     Set up Stable Diffusion model for image generation
#     """
...
def setup_gan_model(ckpt_path, vocab_path):
    """
    Set up GAN model for image generation
    """
...
def generate_with_diffusion(pipe, class_name, num_images=1, img_size=512):
    """
    Generate images using Stable Diffusion
    """
    prompt = f"a high-quality photo of a {class_name}, clear background, professional photography"
    ...
    for i in range(num_images):
        ...
        generated_images.append(image_cv)
    
    return generated_images

def generate_with_gan(model, vocab, class_name, num_images=1, img_size=256):
    """
    Generate images using GAN
    """
    # Convert class name to object and relationship representation
    # This is a simplified example - actual implementation would need proper scene graph
    obj_idx = vocab['object_name_to_idx'].get(class_name, 0)
    
    # Create a simple scene graph with one object
    objs = [obj_idx]
    triplets = []  # No relationships for single object
    
    generated_images = []
    for i in range(num_images):
        # Generate image
        with torch.no_grad():
            imgs, _ = model.forward(objs, triplets)
...
def augment_dataset_gan_diffusion(raw_dataset_path, output_path, target_per_class=900, 
                                 use_diffusion=True, use_gan=False):
    """
    Augment dataset using GAN and Diffusion models
    """
    ...
    if use_gan:
        try:
            # You'll need to provide actual paths to your GAN checkpoint and vocab
            gan_model, vocab = setup_gan_model("path/to/gan/checkpoint.pt", "path/to/vocab.json")
        except Exception as e:
            print(f"Failed to load GAN model: {e}")
            use_gan = False
    
    if not use_diffusion and not use_gan:
        print("No generative models available, using traditional augmentation as fallback")
        return augment_dataset_traditional(raw_dataset_path, output_path, target_per_class)
    ...
    for class_folder in class_folders:
        ...
        # Generate images using GAN/Diffusion
        generated_count = 0
        batch_size = 4  # Generate in batches to manage memory
        
        while generated_count < needed_count:
            current_batch = min(batch_size, needed_count - generated_count)
            
            try:
                if use_diffusion:
                    # Use diffusion model
                    generated_imgs = generate_with_diffusion(
                        diffusion_pipe, class_folder, num_images=current_batch
                    )
                elif use_gan:
                    # Use GAN model
                    generated_imgs = generate_with_gan(
                        gan_model, vocab, class_folder, num_images=current_batch
                    )
                
                # Save generated images
                for i, img in enumerate(generated_imgs):
                    output_filename = f"{class_folder}_gen_{generated_count + i:04d}.jpg"
                    output_filepath = os.path.join(output_class_path, output_filename)
                    cv2.imwrite(output_filepath, img)
                
                generated_count += len(generated_imgs)
                print(f"Generated {generated_count}/{needed_count} images for {class_folder}")
                    
            except Exception as e:
                print(f"Error generating images for {class_folder}: {e}")
                # Fall back to traditional augmentation if generation fails
                if generated_count == 0:
                    print(f"Falling back to traditional augmentation for {class_folder}")
                    augment_class_traditional(class_path, output_class_path, class_images, needed_count)
                    break
        
        print(f"Completed augmentation for {class_folder}. Total images: {existing_count + generated_count}")

def augment_dataset_traditional(raw_dataset_path, output_path, target_per_class=900):
    """
    Traditional augmentation as fallback
    """
    os.makedirs(output_path, exist_ok=True)
    
    # Simple traditional augmentation pipeline
    class_folders = sorted([f for f in os.listdir(raw_dataset_path) 
                          if os.path.isdir(os.path.join(raw_dataset_path, f))])
    
    print(f"Found {len(class_folders)} classes to augment (traditional)")
    ...
    for class_folder in class_folders:
        ...
        augment_class_traditional(class_path, output_class_path, class_images, target_per_class)

def augment_class_traditional(class_path, output_class_path, class_images, target_count):
    """
    Traditional augmentation for a single class
    """
    existing_count = len(class_images)
    needed_count = target_count - existing_count
    
    if needed_count <= 0:
        print(f"Class already has {existing_count} images, skipping augmentation")
        return
    
    # Copy existing images
    for i, img in enumerate(class_images):
        output_filename = f"{os.path.basename(class_path)}_orig_{i:04d}.jpg"
        output_filepath = os.path.join(output_class_path, output_filename)
        cv2.imwrite(output_filepath, img)
    
    # Simple augmentations
    aug_count = 0
    while aug_count < needed_count:
        for base_img in class_images:
            ...
            try:
                ...
                if random.random() > 0.5:
                    # Flip
                    aug_img = cv2.flip(base_img, random.choice([0, 1, -1]))
                else:
                    # Rotate
                    angle = random.randint(-45, 45)
                    h, w = base_img.shape[:2]
                    M = cv2.getRotationMatrix2D((w/2, h/2), angle, 1.0)
                    aug_img = cv2.warpAffine(base_img, M, (w, h))
                ...
                output_filename = f"{os.path.basename(class_path)}_aug_{aug_count:04d}.jpg"
                output_filepath = os.path.join(output_class_path, output_filename)
                cv2.imwrite(output_filepath, aug_img)
                
                aug_count += 1
                if aug_count % 100 == 0:
                    print(f"Generated {aug_count} augmented images for {os.path.basename(class_path)}")
                    
            except Exception as e:
                print(f"Error augmenting image: {e}")
                continue
    
    print(f"Completed traditional augmentation for {os.path.basename(class_path)}. Total images: {existing_count + aug_count}")
...
def main():
    ...
    # 4. Data augmentation using GAN/Diffusion
    augment_dataset_gan_diffusion(
        raw_dataset_path, 
        augmented_path, 
        target_per_class=900,
        use_diffusion=True,  # Use Stable Diffusion
        use_gan=False        # Don't use GAN (set to True if you have a trained GAN model)
    )
